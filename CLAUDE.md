...
- ✅ **Rapport loyers HTML** — bouton **« 📰 Rapport loyers .html »** dans Finances → Parc immobilier. Modale de configuration (période 12mois/exercice/tout, sections cochables, destinataire libre, analyse IA optionnelle) → ouvre `/rapport-loyers?p=...&sec=...&dest=...&analyse=1`. Sections (9, dont 3 obligatoires : Couverture / Synthèse exécutive / Signature) : Tableau de bord des impayés (par locataire avec mois impayés, montant dû, relances), Synthèse par bien, Synthèse mensuelle (12 mois), **Suivi TEOM** (matrice locataire × mois ordures ménagères), **Suivi gaz** (idem), Historique des relances, Fiches détaillées par locataire (annexe). Calculs purs dans `src/lib/rapport-loyers.ts`. Analyse IA via `POST /api/rapport-loyers/analyse` (Claude Sonnet 4.6, prompt « gestionnaire de patrimoine communal »). S'appuie sur la ventilation des charges (`Quittance.chargesOrdures/Gaz/Autres`, `Bail.charges*Mensuelles`) et le modèle `Relance` (date, canal, contenu, résultat). Permission `finance.view-all` ; saisie des relances `parc.manage-relances` (gestionnaire+).
- ✅ **Rapport financier HTML** — bouton **« 📰 Rapport financier .html »** dans Finances → Budget (à côté des exports Excel). Modale de configuration (sections cochables + destinataire libre, choix mémorisés en `localStorage`) → ouvre `/rapport-financier?sec=...&dest=...` (page HTML imprimable, même design que `/rapport`). Sections (14, dont 3 obligatoires : Couverture / Synthèse exécutive / Signature) : Points d'attention (alertes seuils DGFiP), Exécution budgétaire, Ratios R. 2313-1 (3 ans), Évolution pluriannuelle, **Trésorerie** (solde 515, évolution mensuelle, durée de couverture), Contrôle paiements fournisseurs (retards, top 10), Patrimoine & loyers (recouvrement + quittances impayées), **RH & masse salariale** (effectifs, ratio MS/RRF), Projets d'investissement, Subventions, Projection N+5. Calculs purs dans `src/lib/rapport-financier.ts`. Usage typique : suivi maire, demande de prêt bancaire, dossier de subvention. Permission `finance.view-all`. **Analyse IA (optionnelle, cochée par défaut)** : case « 📝 Inclure l'analyse commentée par IA » dans la modale → appel `POST /api/rapport-financier/analyse` (Claude Sonnet 4.6, `ANTHROPIC_API_KEY`) au montage de la page rapport. Le prompt système positionne Claude comme expert-comptable spécialiste M14, ton hybride technique + définitions brèves entre parenthèses, comparaisons aux seuils DGFiP. Sortie structurée JSON `{ synthese, commentaires: { att, bud, ... } }` injectée dans une nouvelle section en tête (synthèse globale) + un encart **📝 Analyse** terra-coloré à la fin de chaque section optionnelle. Coût indicatif : ~5–15 centimes/rapport. Latence : 5–15 s.
